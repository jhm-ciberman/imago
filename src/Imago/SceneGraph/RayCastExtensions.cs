...
public struct HitInfo
{
    ...
    public IMeshData Mesh { get; set; }
    public int TriangleIndex { get; set; }

    public HitInfo(IMeshData mesh, int triangleIndex, float distance, Vector3 normal, Vector3 position)
    {
        this.Mesh = mesh;
...
public static class RayCastExtensions
{
    ...
    public static bool RayCast(this IMeshData mesh, Ray ray, out HitInfo hitInfo)
    {
        hitInfo = new HitInfo();
        hitInfo.Mesh = mesh;
    ...
    /// <returns>True if the <see cref="Ray"/> intersects the mesh; false otherwise</returns>
    public static bool RayCast(this IMeshData mesh, Ray ray, List<HitInfo> hitInfos)
    {
        hitInfos.Clear();
        for (int i = 0; i < mesh.Indices.Length - 2; i += 3)
        {
