...
namespace Sebastian.Geometry
{
	...
	public class Triangulator
    {
        ...
        public Triangulator(Polygon polygon)
        {
            ...
            tris = new ushort[(totalNumVerts - 2) * 3];
                ...
                for (int i = 0; i < vertsInClippedPolygon.Count; i++)
                {
                    ...
                    if (vertexNode.Value.isConvex)
                    {
                        if (!TriangleContainsVertex(prevVertexNode.Value, vertexNode.Value, nextVertexNode.Value))
                        {
                            ...
                            tris[triIndex * 3 + 2] = (ushort) prevVertexNode.Value.index;
                            tris[triIndex * 3 + 1] = (ushort) vertexNode.Value.index;
                            tris[triIndex * 3 + 0] = (ushort) nextVertexNode.Value.index;
                            triIndex++;
                ...
                while (currentNode != null)
                {
                    ...
                    if (p0.X > holeData.bridgePoint.X || p1.X > holeData.bridgePoint.X)
                    {
                        ...
                        if (p0.Y > holeData.bridgePoint.Y != p1.Y > holeData.bridgePoint.Y)
                        {
                            ...
                            if (! this._Approximately(p0.X, p1.X))
                            {
                            ...
                            if (rayIntersectX > holeData.bridgePoint.X)
                            {
                                ...
                                bool isDuplicateEdge = this._Approximately(rayIntersectX, rayIntersectPoint.X);
								...
								var prev = potentialNewBridgeNode.Previous ?? currentNode;
                                bool connectToThisDuplicateEdge = holeData.bridgePoint.Y > prev.Value.position.Y;
