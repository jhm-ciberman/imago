...
// Original source:
// https://github.com/veldrid/veldrid/blob/master/src/Veldrid.StartupUtilities/VeldridStartup.cs
// License: MIT License
...
public static class VeldridStartup
{
    ...
    private static (int Major, int Minor) GetMaxGLVersion(bool gles)
    {
        lock (s_glVersionLock)
        {
            ...
            if (maxVer == null)
            {
                ...
                if (gles) { s_maxSupportedGLESVersion = maxVer; } else { s_maxSupportedGLVersion = maxVer; }
            }
    ...
    private static unsafe bool TestIndividualGLVersion(bool gles, int major, int minor)
    {
        ...
        if (window.NativePointer == nint.Zero || !string.IsNullOrEmpty(errorString))
        {
            Sdl2Native.SDL_ClearError();
