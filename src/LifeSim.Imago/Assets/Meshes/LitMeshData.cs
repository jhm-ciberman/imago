...
/// <summary>
/// Represents mesh data with per-vertex lighting, extending <see cref="BasicMeshData"/> with light information.
/// </summary>
public class LitMeshData : BasicMeshData
{
    /// <summary>
    ...
    /// <inheritdoc/>
    public override VertexFormat VertexFormat => LitVertex.VertexFormat;

    /// <summary>
    /// Initializes a new instance of the <see cref="LitMeshData"/> class.
    /// </summary>
    ...
    public LitMeshData(ushort[] indices, Vector3[] positions, Vector3[]? normals, Vector2[] texCoords, Vector2[] lights)
    ...
    public override DeviceBuffer CreateVertexBuffer(GraphicsDevice gd)
    {
        ...
        LitVertex[] vertices = ArrayPool<LitVertex>.Shared.Rent(this.Positions.Length);
        ...
        uint sizeInBytes = (uint)(this.Positions.Length * Unsafe.SizeOf<LitVertex>());
        DeviceBuffer vertexBuffer = gd.ResourceFactory.CreateBuffer(new BufferDescription(sizeInBytes, BufferUsage.VertexBuffer));
        gd.UpdateBuffer(vertexBuffer, 0, new ReadOnlySpan<LitVertex>(vertices, 0, this.Positions.Length));

        ArrayPool<LitVertex>.Shared.Return(vertices);
        return vertexBuffer;
    }
}
